...
This is of particular interest given results from Arabidopsis that suggest that duplicated regulatory networks will tend to diverge and form parallel regulatory networks that are distinct in terms of their spatiotemporal expression[@blanc_functional_2004].
...
Analysing the regulatory patterns exhibited by *BnSOC1* genes, suggest that some copies respond to the vernalization treatment, while others do not.
...
Although the leaf is the primary expression domain of *FT*, expression of the gene has also been observed in the shoot apex and the hypocotyl of long day grown plants[@kardailsky_activation_1999; @kobayashi_pair_1999], although the biological relevance of these observations are unknown.
...
Potentially the reason the additional copies of *BnFT* are not present in the Darmor-*bzh* reference genome is due to a genome assembly error, caused by the inverted blocks failing to be resolved.
...
These findings suggests that the *BnFT* genes may indeed have diverged in their spatial expression domains, with *BnFT.A7* and *BnFT.C6* exhibiting expression in both the leaf and the apex, whereas *BnFT.A2* and *BnFT.C2.Random* are only expressed in the leaf.
...
In Arabidopsis, *FT* will increase in expression during long days that are inductive to flowering[@corbesier_ft_2007].
Assuming the same is true in *B.\ napus*, the decrease in expression of *BnFT.A7* and *BnFT.C6* is unexpected.
A potential explanation could be that that the *BnFT* genes have diverged in their target genes.
...
When the magnitude of expression between the copies, *BnSOC1.A3*, *BnSOC1.A5*, and *BnSOC1.C4.Random* exhibit the highest expression levels.
...
At one extreme, the *BnSOC1.A5* and *BnSOC1.C4* genes are expressed ~2\ fold higher post-cold relative to before the treatment.
...
This findings suggests that all copies of *BnSOC1* respond to the cold treatment when it is occurring, but only some copies continue to respond to the treatment when it ends.
As observed in the apex, expression magnitude differences are also observed between the copies in the leaf.
*BnSOC1.A4* and *BnSOC1.C4.Random* exhibit the highest expression levels, with the next most highly expressed copy, *BnSOC1.A5*, expressed 3\ -\ 4 fold lower.
...
In the transcriptomic time series there are six copies of *BnaFD* expressed in the apex, situated on chromosomes A1, A8, Ann, C1, C3, and C7.
...
This data suggests that, aside from the *BnFD.A1* copy, the *BnFD* genes have not diverged significantly from one another.

The expression of the *BnFD* genes exhibits very similar expression patterns as the *FD* gene in Arabidopsis; apex specific growth with an increase in expression during the floral transition[@abe_fd_2005; @wigge_integration_2005].
The timing of the decrease in *FD* expression after the day 67 post sowing time point corresponds with the increase in four *AP1* copies (Figure \ref{figure:226:ap1apex}), as observed in Arabidopsis[@wigge_integration_2005], and also with the increase in *BnLFY* gene expression (Figure \ref{figure:231:lfyapex}), which is also consistent with the direct repression of *FD* by *LFY*[@jaeger_interlocking_2013].
Therefore, five of the six *BnaFD* copies seem to be regulated in a similar manner to *FD* in Arabidopsis.
...
Five of the six *BnaSOC1* genes expressed in the apex exhibit a peak in expression at day 69 (Figure \ref{figure:227:soc1apex}), in agreement with *LFY* being regulated by *SOC1*[@lee_soc1_2008; @moon_analysis_2005].
The expression of certain *BnaAP1* and *BnTFL1* genes is also consistent with *BnaLFY* mediated regulation (Figures \ref{figure:226:ap1apex}, \ref{figure:232:tfl1apex}), as has been observed in Arabidopsis[@hempel_floral_1997; @wagner_transcriptional_1999; @william_genomic_2004; @weigel_developmental_1995].
This evidence suggests that the *BnaLFY* genes are similarly regulated to their homologue in Arabidopsis, and that the regulatory roles elucidated for *LFY* in Arabidopsis seem to be conserved in *B.\ napus*.

The co-regulation of the *BnaLFY* genes is consistent with the gene balance hypothesis[@papp_dosage_2003; @birchler_gene_2012].
...
These findings suggest that potentially the copies of *BnaLFY* have been maintained in the *B.\ napus* genome as their loss, or an alteration of their expression, results in a change in flowering time.
A prediction that arises from this is that a *B.\ napus* plant lacking a copy of *BnaLFY* would have later flowering.
...
Finally, the *BnTFL1.Cnn.Random* copy shows a transient peak of expression towards the end of vernalization, with a continued decrease in expression until the final time point after.
...
An explanation for this decrease may be due to *BnTFL1.C3* only maintaining the inflorescence meristem identity early in development, with this role performed by another gene later in development.
...
However, it is interesting that both *BnTFL1.C3* and the *BnaLFY* genes (Figure \ref{figure:231:lfyapex}) exhibit a decrease in expression at the final time point, given the mutual antagonism of the genes in Arabidopsis.
...
This in turn makes the one *BnSOC1* gene that does not increase during the cold, *BnSOC1.A3.Random*, the best candidate for exhibiting the role of *SOC1* in *B.\ napus*.
...
However, from the observed divergence it is also clear that subfunctionalization, neofuncionalization, or the evolution of responsive backup circuits have also influenced gene retention[@force_preservation_1999; @blanc_functional_2004; @kafri_regulatory_2006; @kafri_transcription_2005; @conant_turning_2008].
These different scenarios could be tested, for example, by identifying lines that have non-functional versions of particular floral integrator genes and investigating how the expression profiles of the remaining floral integrators are different in those lines.
